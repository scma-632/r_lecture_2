...
hist(df_s$Rs, prob=TRUE)
lines(density(df_s$Rs))

library(fitdistrplus)
descdist(df_s$Rs, boot = 1000)
str(df_s)
max(df_s$Rs) 
min(df_s$Rs)
...
# Assuming df_s is your dataframe and Rs is the column of interest
data <- df_s$Rs


# Scale the data if it is not between 0 and 1
data_scaled <- (data - min(data)) / (max(data) - min(data))

# Check for NA or NaN values
if (any(is.na(data_scaled)) || any(is.nan(data_scaled))) {
    stop("Data contains NA or NaN values.")
}

# Fit the beta distribution using method of moments
fit <- fitdist(data_scaled, "beta", method = "mme")
summary(fit)
beta = rbeta(10000, 0.3671504, 1.2725101)
hist(data_scaled)
(500-14)/(1850-14)
...
gofstat(fit)
q = 10000
pbeta(.264, 0.3671504, 1.2725101)
dbeta(.264, 0.3671504, 1.2725101)
====================================================================
